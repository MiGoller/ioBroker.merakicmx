    ...
    if (data.observations) {
        for (let o in data.observations) {
            ...
            if (processDevice) {
                ...
                await setStateReadOnlyValue(`${dpDevice}.clientMac`, "clientMac", "string", "value", device.clientMac, true);
                await setStateReadOnlyValue(`${dpDevice}.ipv4`, "ipv4", "string", "value", String(device.ipv4).split("/")[1], true);
                await setStateReadOnlyValue(`${dpDevice}.ipv6`, "ipv6", "string", "value", String(device.ipv6).split("/")[1], true);
                await setStateReadOnlyValue(`${dpDevice}.ssid`, "ssid", "string", "value", device.ssid, true);
                await setStateReadOnlyValue(`${dpDevice}.rssi`, "rssi", "number", "value", Number(device.rssi), true);
                await setStateReadOnlyValue(`${dpDevice}.os`, "os", "string", "value", device.os, true);
                await setStateReadOnlyValue(`${dpDevice}.manufacturer`, "manufacturer", "string", "value", device.manufacturer, true);
    ...
    if (data.observations) {
        for (let o in data.observations) {
            ...
            if (processDevice) {
                ...
                await setStateReadOnlyValue(`${dpDevice}.rssi`, "rssi", "number", "value", Number(bleDevice.rssi), true);
                await setStateReadOnlyValue(`${dpDevice}.seenTime`, "seenTime", "number", "date", Number(bleDevice.seenEpoch) * 1000, true);

                // //  Location data
                // if ((bleDevice.location) && track_location_bluetooth) {
                //     await setStateReadOnlyValue(`${dpDevice}.latitude`, "latitude", "number", "	value.gps.latitude", Number(bleDevice.location.lat || "0"), true);
                //     await setStateReadOnlyValue(`${dpDevice}.longitude`, "longitude", "number", "value.gps.longitude", Number(bleDevice.location.lng || "0"), true);
                //     await setStateReadOnlyValue(`${dpDevice}.accuracy`, "accuracy", "number", "value", Number(bleDevice.location.unc || "0"), true);
                //     await setStateReadOnlyValue(`${dpDevice}.gps-coordinates`, "gps-coordinates", "string", "value.gps", `${Number(bleDevice.location.lat || "0")},${Number(bleDevice.location.lng || "0")}`, true);
                // }
