    ...
    if (data.observations) {
        for (let o in data.observations) {
            ...
            if (processDevice) {
                ...
                await setStateReadOnlyValue(`${dpDevice}.latitude`, "latitude", "number", "	value.gps.latitude", Number(bleDevice.location.lat), true);
                await setStateReadOnlyValue(`${dpDevice}.longitude`, "longitude", "number", "value.gps.longitude", Number(bleDevice.location.lng), true);
                await setStateReadOnlyValue(`${dpDevice}.accuracy`, "accuracy", "number", "value", Number(bleDevice.location.unc), true);
                await setStateReadOnlyValue(`${dpDevice}.gps-coordinates`, "gps-coordinates", "text", "value.gps", `${Number(bleDevice.location.lat)},${Number(bleDevice.location.lng)}`, true);
